...
def fn(arg):
    num_factors, num_users, num_items, K, users_in_fname, \
...
def main():
    ...
    TOP_K = [1, 5, 10, 50]
    output_suffix = 'lemp-%s-%s/' % (('icc' if args.icc else 'no-icc'), ('simd'
        if args.simd else 'no-simd'))
    ...
    for (model_dir, (num_factors, num_users, num_items)) in TO_RUN:
        for K in TOP_K:
            #run_args.append(
            result = fn(
                    (num_factors, num_users, num_items, K,
                    MODEL_DIR_BASE + model_dir + 'user_weights.csv',
                    MODEL_DIR_BASE + model_dir + 'item_weights.csv',
                    output_dir_base.format(model_dir=model_dir, K=K), runner)
            )
            print(result)
