...
int *indexPermutation(float *upperBounds) {
  ...
  std::sort(indexes, indexes + (numItems + 1), [&](size_t i1, size_t i2) {
...
float *applyPermutation(float *upperBounds, int *permutation) {
  ...
  std::transform(permutation, permutation + (numItems + 1), sortedUpperBounds,
...
void clusterPrep(fullCluster_t *fullClusterData, int clusterID,
                 float *fullUserData, float *clusterCentroids,
                 float *allItemWeights, float *allItemNorms, int numBins, int k,
                 float *vectorized_Acos_output_ic, std::ofstream &misc_file,
  ...
  //
  //    unsigned long long upperBoundCreation_time, sortUpperBound_time,
  // computeTopK_time, makeFreqUnion_time = 0;
  ...
  // userNormTuple_t *userNormTuple_array = (userNormTuple_t
  // *)malloc(sizeof(struct userNormTuple) * numUsersInCluster);
  ...
  for (i = 0; i < numUsersInCluster; i++) {
    ...
    //        userNormTuple_array[i].userNorm = cblas_snrm2(numFeatures,
    // &fullUserData[userNormTuple_array[i].userID*numFeatures], 1);
  ...
  //        float *vectorized_Acos_input_uc2 =
  // computeCosineSimilarityUserCluster(&fullUserData[userNormTuple_array[0].userID*numFeatures],
  // &clusterCentroids[clusterID*numFeatures], numUsersInCluster,
  // userNormTuple_array);
  //
  //    float centroidNorm = cblas_snrm2(numFeatures,
  // &clusterCentroids[clusterID*numFeatures], 1);
  ////
  ////    //float vectorized_Acos_input_uc[numUsersInCluster];
  //    float *vectorized_Acos_input_uc = (float
  // *)malloc(sizeof(float)*numUsersInCluster);
  //    //this is computing the 'x' in acos(x) for all users in the clusters -
  // stored in the same order as the userNormTuple
  //    for (i = 0; i < numUsersInCluster; i++) {
  //        vectorized_Acos_input_uc[i] = cblas_sdot(numFeatures,
  // &clusterCentroids[clusterID*numFeatures], 1,
  // &fullUserData[userNormTuple_array[i].userID*numFeatures], 1) /
  // (centroidNorm*userNormTuple_array[i].userNorm);
  ...
  // float *vectorized_Acos_output_uc = (float
  // *)malloc(sizeof(float)*numUsersInCluster);
  ...
  //        printf("%d: %f \t %f\n", r, vectorized_Acos_input_uc[r],
  // vectorized_Acos_input_uc[r]);
  //    }
  //
  //    if (clusterID == 3) {
  //        exit(1);
  ...
  //    float *vectorized_Acos_input_ic = (float
  // *)malloc(sizeof(float)*(numItems+1));
  //    vectorized_Acos_input_ic[0] = 0.0;
  //    //this is computing the 'x' in acos(x) for all items - stored in the
  // order
  //    for (i = 0; i < (numItems+1); i++) {
  //        vectorized_Acos_input_ic[i] = cblas_sdot(numFeatures,
  // &clusterCentroids[clusterID*numFeatures], 1,
  // &allItemWeights[i*numFeatures], 1) / (centroidNorm*allItemNorms[i]);
  //    }
  //    //float vectorized_Acos_output_ic[numItems+1];
  //    float *vectorized_Acos_output_ic = (float
  // *)malloc(sizeof(float)*(numItems+1));
  //
  //
  //    vsAcos((numItems+1), vectorized_Acos_input_ic,
  // vectorized_Acos_output_ic);

  //    for (i = 0; i < numItems+1; i++) {
  //        printf("ClusterID: %d \t ItemID: %d \t theta_ic: %f\n", clusterID,
  // i, vectorized_Acos_output_ic[i]);
  //    }
  //    return;

  // float const_vector_theta[numItems+1]; //will be the const theta_uc in each
  // bucket as well as intermediary theta_ic - theta_uc, and then
  // cos(theta_ic-theta_uc)
  // float *const_vector_theta = (float *)malloc(sizeof(float)*(numItems+1));
  float *const_vector_theta =
      (float *)mkl_malloc(sizeof(float) * (numItems + 1), 64);
  ...
  for (i = 0; i < numBins; i++) {
    ...
    vsSub((numItems + 1), vectorized_Acos_output_ic, const_vector_theta,
          const_vector_theta);
  ...
  // will have two arrays - one of sorted upperbounds -- and one of
  // corresponding index (i.e. itemID)
  ...
  for (i = 0; i < numBins; i++) {
    sortedUpperBounds[i] = (upperBoundItem_t *)mkl_malloc(
        (numItems + 1) * sizeof(struct upperBoundItem), 64);
  ...
  for (i = 0; i < numBins; i++) {
    ippsSortRadixDescend_64s_I_L((Ipp64s *)sortedUpperBounds[i], (numItems + 1),
                                 pBuffer);
  }
  ...
  //    float **NewupperBounds = (float **)mkl_malloc(sizeof(float*)*numBins,
  // 64);
  //    if (NewupperBounds == NULL) {
  //        printf( "\n ERROR: Can't allocate memory for NewupperBounds.
  // Aborting... \n\n");
  ...
  //
  //    for (i = 0; i < numBins; i++){
  //        NewupperBounds[i] = (float *)mkl_malloc((numItems+1) *
  // sizeof(float), 64);
  //        if (NewupperBounds[i] == NULL) {
  //            printf( "\n ERROR: Can't allocate memory for NewupperBounds[i].
  // Aborting... \n\n");
  ...
  //    int *indexes = (int *)malloc(sizeof(int)*(numItems+1));
  //    for (i = 0; i < numBins; i++) {
  //        ippsSortIndexDescend_32f_I((Ipp32f*) upperBounds[i], indexes,
  // (numItems+1));
  //        for (j = 0; j < (numItems+1); j++) {
  ...
  //        sortedUpperBounds[i] = applyPermutation(upperBounds[i],
  // indexPermutations[i]);
  //    }
  //
  //    call_stop = _rdtsc3();
  ...
  //    call_start = _rdtsc3();
  //    upperBoundItem_t **sortedUpperBounds = (upperBoundItem_t
  // **)malloc(numBins * sizeof(upperBoundItem_t *));
  ...
  //    }
  //    for (i = 0; i < numBins; i++){
  //        sortedUpperBounds[i] = (upperBoundItem_t *)malloc((numItems+1) *
  // sizeof(struct upperBoundItem));
  //        if (sortedUpperBounds[i] == NULL) {
  //            printf( "\n ERROR: Can't allocate memory for newUpperBounds[i].
  // Aborting... \n\n");
  ...
  //    for (i = 0; i < numBins; i++) {
  //        std::sort(sortedUpperBounds[i], sortedUpperBounds[i]+(numItems+1),
  // compareByBound);
  //
  //    }
  //    call_stop = _rdtsc3();
  ...
  for (i = 0; i < numUsersInCluster; i++) {
    ...
    //        cblas_sgemv(CblasRowMajor, CblasNoTrans, m, n, alpha, userNorms,
    // n, &fullUserData[currentUser*numFeatures], stride, beta,
    // usersDOTcentroid, stride);
    ...
    for (j = 0; j < k + 1; j++) {  // add 1 because first item is junk
      ...
      //            float true_ui = acosf(score /
      // (userNormTuple_array[i].userNorm*allItemNorms[itemID]));
      q.push(std::make_pair(score, itemID));
      //            printf("ClusterID: %d \t UserID: %d \t ItemID: %d \t
      // theta_ic: %f \t theta_uc: %f \t ic-uc: %f \t ui: %f\n", clusterID,
      // currentUser, itemID, vectorized_Acos_output_ic[itemID],
      // vectorized_Acos_output_uc[i], vectorized_Acos_output_ic[itemID] -
      // vectorized_Acos_output_uc[i], true_ui);
    }
    int itemsVisited = k;
    ...
    //        printf("%d,%d,%d,%f\n", clusterID, currentUser, itemsVisited,
    // vectorized_Acos_output_uc[i]);

    //            misc_file << k << "," << clusterID << "," << currentUser <<
  ...
  //
  //
  //        std::vector<std::pair<int, int>> tuples(freqMap.begin(),
  // freqMap.end());
  //        std::sort(tuples.begin(), tuples.end(), [](const std::pair<int,int>
  // &left, const std::pair<int,int> &right) {
  //            return left.second > right.second;
  //        });
  //
  //        call_stop = _rdtsc3();
  //        call_run_time = call_stop - call_start;
  //        //printf("Cluster: %d \t time to compute union sorted by freq
  // [K=%d]: %llu\n", clusterID, k, call_run_time);
  //        makeFreqUnion_time = call_run_time;
  //
  ////        run_file << clusterID << "," << k << "," <<
  ///upperBoundCreation_time << "," << sortUpperBound_time << "," <<
  ///computeTopK_time << "," << makeFreqUnion_time << "\n";
  //
  for (i = 0; i < numUsersInCluster; i++) {
    free(allTopK[i]);
  }
  ...
  // ----------All TopK computed, union Below---POTENTIALLY TOO
  // SLOW------------------
  ...
  //
  //    it=std::set_union (allTopK[0],allTopK[0]+k, allTopK[1],allTopK[1]+k,
  // v.begin());
  //    v.resize(it-v.begin());
  //
  //    for (i = 2; i < numUsersInCluster; i++) {
  //        std::sort (allTopK[i],allTopK[i]+k);
  //        it=std::set_union (v.begin(), v.end(), allTopK[i],allTopK[i]+k,
  // v.begin());
  //        v.resize(it-v.begin());
  //    }
  ...
  // ----------Save Cluster TopK List (sorted by frequency)
  // Below------------------
