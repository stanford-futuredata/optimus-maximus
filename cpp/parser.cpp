...
// returns a struct containing an array of all cluster info (stored as |cluster_id|numUsers|users|) and an offset array
// which is indexed by clusterID and returns the starting index of the cluster in the data array.
...
fullCluster_t *parseClusers(FILE *cluserIDsInverted, FILE *cluserUserCounts){
    ...
    int *allClusters = (int*)malloc(sizeof(int)*(numUsers + (2*numClusters)));
    ...
    fullCluster_t *fullClusterData = (fullCluster_t*)malloc(sizeof(struct fullCluster));
    fullClusterData->clusterArray = allClusters;
    fullClusterData->clusterOffset = clusterOffset; //offset points to start of cluster block (i.e. the cluster_id slot)
...
// returns a struct containing an array of all user vectors (stored as array of floats) and an offset array
// which is indexed by userID and returns the starting index of the user vector in the data array.
...
//    outputFile = "newUserMappingSTAGE2_" + std::to_string(numberUsers) + ".txt";
//
...
// returns an array of size numberItems*numFeature floats. Index into the array by
...
//    float *allItems = (float *)malloc(sizeof(float)*(numItems+1)*numFeatures);
...
//    float *allCentroids = (float *)malloc(sizeof(float)*(numClusters)*numFeatures);
    
    float *allCentroids = (float *)mkl_malloc(sizeof(float)*(numClusters)*numFeatures, 64);
...
// Going to make a new user array where users are arranged in cluster order. Needed for usersInClusters X Centroid Matrix-Vector Prod.
float *reorderUsersInClusterOrder(float *allUsers, fullCluster_t *fullClusterData){
    
//    float *allUsers = (float *)malloc(sizeof(float)*numberUsers*numFeatures);
    float *allUsersNew = (float *)mkl_malloc(sizeof(float)*(numberUsers)*numFeatures, 64);
    if (allUsersNew == NULL) {
...
//    std::ofstream myfile;
//    std::string outputFile;
//    outputFile = "newUserMappingSTAGE1_" + std::to_string(numberUsers) + ".txt";
//    
//    myfile.open (outputFile);
//    myfile << "Original User ID, New User ID\n";
    ...
    for (i = 0; i < numClusters; i++) {
        ...
        for (j = 0; j < numUsersInCluster; j++) {
            ...
            for (k = 0; k < numFeatures; k++) {
                allUsersNew[(newUserNumber*numFeatures)+k] = allUsers[(oldUserNumber*numFeatures)+k];
            }
