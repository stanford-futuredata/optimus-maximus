...
int main(int argc, const char * argv[]) {
    ...
    if (argc != 7) {
        printf("Usage: naive <numItems> <numLatentFactors> <numUsers> <K> <UserWeight File Name> <ItemWeight File Name> \n");
        exit(1);
    }
...
//    myfile.open ("naive_output.txt", std::ofstream::out | std::ofstream::app);
    ...
    for (int i = 0; i < numUsers; i++) {
        ...
        for (int j = 0; j < numItems; j++) {
            userItemScores.push_back({cblas_sdot(numFeatures, &allUsers[i][1], 1, &allItems[j*numFeatures], 1), j});
        }
        std::sort(userItemScores.begin(), userItemScores.end(), [](const std::pair<int,int> &left, const std::pair<int,int> &right) {
