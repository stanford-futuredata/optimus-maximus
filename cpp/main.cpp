...
int main(int argc, const char *argv[]) {
  ...
  if (argc != 12) {  // MAKE SURE TO FIX FOR USER TO CLUSTER
    printf(
        "Usage: fomo <numItems> <numLatentFactors> <numClusters> <numBins> <K> "
        "<numClusters to Time> <Weight Directory> <Cluster Directory> "
        "<MaxThreads> <SampleSize> <Iters>\n");
    exit(1);
  }
  ...
  if (((char)allArgs[8].back()) == '/') {
    clusterUserCountsPath =
        allArgs[8] + allArgs[3] + "_user_cluster_ids_counts";
    clustersToUsersPath =
        allArgs[8] + allArgs[3] + "_user_cluster_ids_inverted";
  ...
  std::unordered_map<int, int> userClusterMap =
      userToClusterMap(userToClusters);
  ...
  float *cosineSimilarityItemCentroid =
      computeCosineSimilarityItemsCentroids(allItems, allCentroids);
  float *arcCosineSimilarityItemCentroid = (float *)mkl_malloc(
      sizeof(float) * numberClusters * (numberItems + 1), 64);
  if (arcCosineSimilarityItemCentroid == NULL) {
    printf(
        "\n ERROR: Can't allocate memory for arcCosineSimilarityItemCentroid. "
        "Aborting... \n\n");
  ...
  vsAcos(remaining, &cosineSimilarityItemCentroid[first],
         &arcCosineSimilarityItemCentroid[first]);
  // printf("here\n");
  ...
  //    int clustersToVisit[10] = {3017, 2981, 346, 1996, 3745, 675, 937, 3516,
  // 1958, 1497};

  std::ofstream misc_file;
  std::string misc_out_file;
  misc_out_file = "output_U-" + std::to_string(numberUsers) + "_F-" +
                  std::to_string(numberFeatures) + "_C-" +
                  std::to_string(numberClusters) + "_B-" +
                  std::to_string(numberBins) + ".csv";
  ...
  run_out_file = "runtime_U-" + std::to_string(numberUsers) + "_F-" +
                 std::to_string(numberFeatures) + "_C-" +
                 std::to_string(numberClusters) + "_B-" +
                 std::to_string(numberBins) + ".txt";
  //    run_file.open (run_out_file, std::ofstream::out | std::ofstream::app);
  //
  //    run_file << "PreComp Time:" << call_run_time << "\n";
  //    run_file <<
  // "Cluster,K,UpperBoundCreation,SortUpperBound,TopK,FreqMap\n";
  ...
  for (int i = 0; i < numberClusters; i++) {
    clusterPrep(allClusters, i, allUsers, allCentroids, allItems, allItemNorms,
                numberBins, k,
                &arcCosineSimilarityItemCentroid[i * (numberItems + 1)],
                misc_file, run_file);
  }
  time_end = dsecnd();
  ...
  outputFile = "simdex2_u" + std::to_string(numberUsers) + "_f" +
               std::to_string(numFeatures) + "_k" + std::to_string(k) + "_c" +
               std::to_string(numClusters) + ".csv";
  myfile.open(outputFile, std::ofstream::out | std::ofstream::app);
  //    myfile <<
  // "Users,Clusters,Features,K,NumBins,ClusterLoadTime,UserLoadTime,CompTime,UpperBoundCreationTime,UpperBoundSortTime,WalkTime,SampleSize,Iters"
  // << std::endl;
  myfile << numberUsers << "," << numClusters << "," << numFeatures << "," << k
         << "," << numberBins << "," << time_diff3 << "," << time_diff1 << ","
         << time_diff2 << "," << creationTime << "," << sortTime << ","
         << computeKTime << "," << sampleSize << "," << iterations << std::endl;

  //    myfile << "Users: " << numberUsers << " Clusters: " << numClusters << "
  // Features: " << numFeatures << " K: " << k << " NumBins: " << numberBins <<
  // "  " << " Cluster Load Time: " << time_diff3 << " User Load Time: " <<
  // time_diff1 << " Comp Time: " << time_diff2 << " UpperBoundCreationTime: "
  // << creationTime << " UpperBoundSortTime: " << sortTime << " WalkTime: " <<
  // computeKTime << "\n";
  myfile.close();
