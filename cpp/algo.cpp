...
void computeTopKForCluster(
    ...
    const int num_latent_factors, const int K,
  ...
  ippsThreshold_32f_I((Ipp32f *)upper_bounds, num_items,
  ...
  ippsSortRadixIndexDescend_32f(
      (Ipp32f *)upper_bounds, src_stride_bytes,
      (Ipp32s *)sorted_upper_bounds_indices, num_items,
      pBuffer);
  ...
  for (int j = 0; j < batch_size; j++) {
    ...
    cblas_dcopy(
        ...
        &sorted_item_weights[j * num_latent_factors],
        1);
  }
  int batch_counter = batch_size;
...
// ----------Computer Per User TopK Below------------------
  ...
  for (int user_id = 0; user_id < num_users_to_compute; user_id++) {
#ifdef STATS
    dsecnd();
    time_start = dsecnd();
#endif
    ...
    cblas_dgemv(CblasRowMajor, CblasNoTrans, m, k, alpha,
                sorted_item_weights, k,
                &user_weights[user_id * num_latent_factors], stride, beta,
                user_dot_items, stride);

#pragma simd
    for (int j = 0; j < batch_size; j++) {
      user_norm_times_upper_bound[j] =
          user_norms[user_id] * sorted_upper_bounds[j];
    }
    ...
    for (int j = K; j < num_items; j++) {
      if (batch_counter == j) {
        ...
        const int true_batch_size =
            std::min(batch_size, num_items - batch_counter);
        ...
        for (int l = 0; l < true_batch_size; l++) {
          item_id = sorted_upper_bounds_indices[l];
          sorted_upper_bounds[l] = upper_bounds[item_id];
          cblas_dcopy(num_latent_factors,
                      &item_weights[item_id * num_latent_factors], 1,
                      &sorted_item_weights[l * num_latent_factors],
                      1);
        }
        batch_counter += true_batch_size;
      }

      if ((j & mod) == 0) {
        cblas_dgemv(CblasRowMajor, CblasNoTrans, m, k, alpha,
                    &sorted_item_weights[j * num_latent_factors],
                    k, &user_weights[user_id * num_latent_factors], stride, beta,
                    user_dot_items, stride);

#pragma simd
        for (int l = 0; l < batch_size; l++) {
          user_norm_times_upper_bound[l] =
              user_norms[user_id] * sorted_upper_bounds[j + l];
        }
      }
    ...
    for (int j = 0; j < K; j++) {
      ...
      top_K_items[user_id * K + K - 1 - j] = p.second;  // store item ID
      q.pop();
    }
#ifdef STATS
    time_end = dsecnd();
...
#ifdef DEBUG
    std::cout << "User ID " << user_ids_in_cluster[user_id] << std::endl;
    check_against_naive(&user_weights[user_id * num_latent_factors], item_weights,
                        num_items, num_latent_factors, &top_K_items[user_id * K],
                        top_K_scores, K);
#endif
#ifdef STATS
    const double total_user_time_ms =
        1000 * (user_top_K_time + batch_time + sort_time + upper_bounds_time);
    user_stats_file << cluster_id << "," << theta_ucs[user_id] << ","
