...
float *parseAllItems(FILE *itemWeights){
    ...
    float *allItems = (float *)mkl_malloc(sizeof(float)*(numItems+1)*numFeatures, 64);
    if (allItems == NULL) {
...
//    myfile << "Original User ID, New User ID\n";
    ...
    float *allUsers = (float *)mkl_malloc(sizeof(float)*numUsers*numFeatures, 64);
    if (allUsers == NULL) {
...
//        allUserNorms[i] = cblas_snrm2(numFeatures, &allUserWeights[i*numFeatures], 1);
//    }
//    
//    return allUserNorms;
//}
...
int main(int argc, const char * argv[]){
    ...
    if (argc != 7) {
        printf("Usage: cross <numItems> <numUsers> <numLatentFactors> <k> <Weight Directory> <mkl_cores> \n");
        exit(1);
    }
...
//        vsMul(numFeatures, &allItems[i*numFeatures], &allItemNorms[i*numFeatures], &allItems[i*numFeatures]);
//    }
//    for (i = 0; i < numUsers; i++) {
//        vsMul(numFeatures, &allUsers[i*numFeatures], &allUserNorms[i*numFeatures], &allUsers[i*numFeatures]);
//    }
...
//                    (2), n, k, alpha, allUsers, k, allItems, k, beta, cosineSimilarity, n);
        
//        std::ofstream myfile;
//        myfile.open ("naive_output.txt", std::ofstream::out | std::ofstream::app);
...
//                if (j < 100) {
//                    q.push(std::make_pair(cosineSimilarity[(i*(numItems+1))+j], j));
//                    continue;
//                }
...
//        }
        ...
        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans,
                    (per_instance), n, k, alpha, allUsers, k, allItems, k, beta, cosineSimilarity, n);
        ...
        time_st = dsecnd();
        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans,
                    (per_instance), n, k, alpha, &allUsers[per_instance], k, allItems, k, beta, cosineSimilarity, n);
        time_end = dsecnd();
        time_avg1 += (time_end - time_st);
        ...
        time_st = dsecnd();
        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans,
                    (per_instance), n, k, alpha, &allUsers[per_instance*2], k, allItems, k, beta, cosineSimilarity, n);
        time_end = dsecnd();
        time_avg1 += (time_end - time_st);
...
//        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans,
//                    (per_instance), n, k, alpha, &allUsers[per_instance*3], k, allItems, k, beta, cosineSimilarity, n);
//        
//        computeTopK(cosineSimilarity, per_instance);
        
//        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans,
//                    (per_instance), n, k, alpha, &allUsers[per_instance*4], k, allItems, k, beta, cosineSimilarity, n);
//        
//        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans,
//                    (per_instance), n, k, alpha, &allUsers[per_instance*5], k, allItems, k, beta, cosineSimilarity, n);
//        
//        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans,
//                    (per_instance), n, k, alpha, &allUsers[per_instance*6], k, allItems, k, beta, cosineSimilarity, n);
//        
//        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans,
//                    (per_instance), n, k, alpha, &allUsers[per_instance*7], k, allItems, k, beta, cosineSimilarity, n);
//        
//        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans,
//                    (per_instance), n, k, alpha, &allUsers[per_instance*8], k, allItems, k, beta, cosineSimilarity, n);
//        
//        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans,
//                    (per_instance), n, k, alpha, &allUsers[per_instance*9], k, allItems, k, beta, cosineSimilarity, n);
...
//        float *cosineSimilarity = (float *)mkl_malloc(sizeof(float)*(numUsers)*(numItems+1), 64);
        
        float *cosineSimilarity = (float *)mkl_malloc(needed, 64);
        if (cosineSimilarity == NULL) {
            printf( "\n ERROR: Can't allocate memory for cosineSimilarity. Aborting... \n\n");
            mkl_free(cosineSimilarity);
    //        cosineSimilarity = (float *)malloc(sizeof(float)*numUsers*(numItems+1));
    //        if (cosineSimilarity == NULL) {
    //            printf( "\n ERROR: Can't allocate memory for cosineSimilarity. Aborting... \n\n");
...
//        call_start = _rdtsc2();
        ...
        time_st = dsecnd();
        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans,
                    m, n, k, alpha, allUsers, k, allItems, k, beta, cosineSimilarity, n);
        time_end = dsecnd();
        time_avg1 = (time_end - time_st);
        printf("gemm time: %f secs \n", time_avg1);
...
//        MKL_free(allUserNorms);
    ...
    outputFile = "blocked_u" + std::to_string(numUsers) + "_f" + std::to_string(numFeatures) + "_k" + std::to_string(topk) + ".txt";
    myfile.open (outputFile, std::ofstream::out | std::ofstream::app);
    myfile << "Users: " << numUsers << " Items: " << numItems << " Features: " << numFeatures << " K: " << topk << "\t" << "GEMM Time: " << time_avg1 << " Heap Time: " << time_avg2 << " Total Time: " << time_avg3 << "\n";
    myfile.close();
