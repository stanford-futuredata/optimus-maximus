...
__inline unsigned int Next_Uniform_Int(VSLStreamStatePtr stream, float* D_UNIFORM01_BUF, unsigned int *D_UNIFORM01_IDX, int M, int N) { /* Return integer uniformly distributed on {0,...,m-1}. */
    ...
    if( (*D_UNIFORM01_IDX)==0 ) {
        ...
        vsRngUniform( VSL_RNG_METHOD_UNIFORM_STD, stream, RNGBUFSIZE, D_UNIFORM01_BUF, 0.0, 1.0 );
...
__inline void Fisher_Yates_shuffle(VSLStreamStatePtr stream, float* D_UNIFORM01_BUF, unsigned int *D_UNIFORM01_IDX, unsigned int* PERMUT_BUF, int M, int N) {
    unsigned int i,j;
    unsigned int tmp;
...
#pragma novector /* Vectorization is useless here */
    ...
    for( i=0; i<M; i++ ) {
        ...
        j = Next_Uniform_Int(stream,D_UNIFORM01_BUF, D_UNIFORM01_IDX, M, N); /* Uniform integer from {i,...,N-1} */
...
int *randomArray(int max, int size){
    ...
    float* D_UNIFORM01_BUF; /* Each thread has its own buffer of intermediate uniforms */
    ...
    if( !(PERMUT_BUF=(unsigned int*)mkl_malloc( sizeof(unsigned int)*N, ALIGNMENT_BYTES )) ) {
        ...
        if( !(D_UNIFORM01_BUF=(float*)mkl_malloc( sizeof(float)*RNGBUFSIZE, ALIGNMENT_BYTES )) ) {
            ...
            if( (vslNewStream( &stream, VSL_BRNG_MT2203, seed ))!=VSL_STATUS_OK ) {
                ...
                for( sample_num=0; sample_num<EXPERIM_NUM; sample_num++ ) {
                    ...
                    Fisher_Yates_shuffle(stream,D_UNIFORM01_BUF,&D_UNIFORM01_IDX,PERMUT_BUF, M, N);
                    
                    /* A2.5: (Copy stage) for i from 0 to M-1 do RESULTS_ARRAY[i]=PERMUT_BUF[i] */
                    #pragma simd
                    for( i=0; i<M; i++ ) RESULTS_ARRAY[sample_num*M + i] = PERMUT_BUF[i];
                }
...
//    #pragma simd /* No need for vectorization when printing results sequentially */
//    for( sample_num=0; sample_num<EXPERIM_NUM; sample_num++ ) {
...
//        for( i=4563; i<4583; i++ ) printf("%2d, ", (int)RESULTS_ARRAY[sample_num*M+i]);
//        printf("\n");
//    }
