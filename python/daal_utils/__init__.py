...
def createSparseTable(dataset_filename, ntype=np.float64):
    ...
    if (num_columns != num_nonzeros
            or num_nonzeros != int(row_offsets[num_vectors]) - 1
            or num_features == 0 or num_vectors == 0):
        print("Incorrect format of file")
        exit()

    numeric_table = CSRNumericTable(data, col_indices, row_offsets,
                                    num_features, num_vectors)

    return numeric_table


def printAprioriItemsets(large_itemsets_table,
                         large_itemsets_support_table,
    ...
    large_itemsets = large_itemsets_table.getBlockOfRowsAsDouble(
        0, num_items_in_large_itemsets).flatten()
    large_itemsets_support_data = large_itemsets_support_table.getBlockOfRowsAsDouble(
        0, large_itemset_count).flatten()

    large_itemsets_array = [[] for x in range(large_itemset_count)]

    for i in range(num_items_in_large_itemsets):
        large_itemsets_array[int(large_itemsets[2 * i])].append(
            large_itemsets[2 * i + 1])

    support_array = [0] * large_itemset_count

    for i in range(large_itemset_count):
        support_array[int(large_itemsets_support_data[
            2 * i])] = large_itemsets_support_data[2 * i + 1]
    ...
    for i in range(i_min, large_itemset_count):
        ...
        print(
            "{:.0f}}}\t\t".format(
                large_itemsets_array[i][len(large_itemsets_array[i]) - 1]),
            end='')

        print("{:.0f}".format(support_array[i]))


def printAprioriRules(left_items_table,
                      right_items_table,
                      confidence_table,
    ...
    left_items = left_items_table.getBlockOfRowsAsDouble(
        0, num_left_items).flatten()
    right_items = right_items_table.getBlockOfRowsAsDouble(
        0, num_right_items).flatten()
    confidence = confidence_table.getBlockOfRowsAsDouble(0,
                                                         num_rules).flatten()
    ...
    for i in range(num_right_items):
        right_items_array[int(right_items[2 * i])].append(
            right_items[2 * i + 1])
    ...
    for i in range(i_min, num_rules):
        ...
        print(
            "{:.0f}}} => {{".format(
                left_items_array[i][len(left_items_array[i]) - 1]),
            end='')
...
def printArray(array,
    ...
    for i in range(num_printed_rows):
        for j in range(num_printed_cols):
            print(
                "{:<{width}.{dec}f}".format(
                    flat_array[i * num_cols + j], width=interval,
                    dec=decimals),
                end='')
        print()
    print()
...
def isUpper(layout):
    if (layout == NumericTableIface.upperPackedSymmetricMatrix
            or layout == NumericTableIface.upperPackedTriangularMatrix):
        return True
    return False
...
def isLower(layout):
    if (layout == NumericTableIface.lowerPackedSymmetricMatrix
            or layout == NumericTableIface.lowerPackedTriangularMatrix):
        return True
    return False
...
def printNumericTable(data_table,
                      message='',
                      num_printed_rows=0,
                      num_printed_cols=0,
                      interval=10):
...
def printNumericTables(data_table_1,
                       data_table_2,
                       title_1='',
                       title_2='',
                       message='',
    ...
    for i in range(num_rows):
        ...
        for j in range(num_cols_2):
            print(
                "{:<{width}.0f}".format(
                    data_float64_2[i * num_cols_2 + j], width=interval),
                end='')
        print()
    print()
...
def computePartialModelBlocksToNode(nBlocks, dataTable, transposedDataTable,
                                    usersPartition, itemsPartition,
                                    usersOutBlocks, itemsOutBlocks):
    usersPartition[0] = 0
    itemsPartition[0] = 0
    for i in range(nBlocks):
        usersPartition[i +
                       1] = usersPartition[i] + dataTable[i].getNumberOfRows()
        itemsPartition[
            i +
            1] = itemsPartition[i] + transposedDataTable[i].getNumberOfRows()

    for i in range(nBlocks):
        usersOutBlocks[i] = computeOutBlocks(nBlocks, dataTable[i],
                                             itemsPartition)
        itemsOutBlocks[i] = computeOutBlocks(nBlocks, transposedDataTable[i],
                                             usersPartition)
...
def printALSRatings(usersOffset, itemsOffset, ratings):
    ...
    for i in range(nUsers):
        for j in range(nItems):
            print("{}, {}, {:.6g}".format(i + usersOffset, j + itemsOffset,
                                          ratingsData[i * nItems + j]))
...
def printTensor(dataTable,
    ...
    printArray(block.getArray(),
               int(nPrintedCols),
               int(nPrintedRows), int(nCols), message, interval)
...
def printTensors(dataTable1,
    ...
    for i in range(nPrintedRows):
        for j in range(nCols1):
            print(
                "{v:<{width}.0f}".format(
                    v=dataType1[i * nCols1 + j], width=interval),
                end='')

        for j in range(nCols2):
            print(
                "{:<{width}.3f}".format(
                    dataType2[i * nCols2 + j], width=int(interval / 2)),
                end='')
        print()
    print()
...
def printTensor3d(dataTable,
    ...
    for i in range(nFirstDim):
        ...
        printArray(block.getArray(),
                   int(nThirdDim),
                   int(nSecondDim), int(nThirdDim), "", interval)
...
def readTensorFromCSV(datasetFileName):
    ...
    # FIXME we need a proper GC in DAAL tables
    #global gc_protector
    #gc_protector.append(tensorData)
