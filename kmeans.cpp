...
float *computeCentroids(float *sample_users, size_t nCols, int sampleSize){
    ...
    data_management::NumericTablePtr sampleTablePtr = NumericTablePtr(new HomogenNumericTable<float>(sample_users, nCols, sampleSize));
...
//    NumericTable *dataTab = dataTable.get();
    ...
    float *sample_users = (float*)MKL_malloc(sizeof(float)*nCols*sampleSize, 64);
    
    for (int i = 0; i<sampleSize; i++) {
        cblas_scopy(nCols, &inputArray[(randUsers[i])*nCols], 1, &sample_users[i*nCols], 1);
    }
    ...
    daal::services::interface1::ObjectDeleter<daal::algorithms::Analysis<kmeans::Batch<float>>>(algorithm2);
